"""
配置管理 - 从 .env 加载配置
QQ邮箱 IMAP/SMTP、阿里百炼（DashScope）LLM、调度间隔等
"""
...
class Settings(BaseSettings):
    ...
    # 阿里百炼（DashScope）LLM 配置
    DASHSCOPE_API_KEY: str = ""
    DASHSCOPE_BASE_URL: str = "https://dashscope.aliyuncs.com/compatible-mode/v1"
    DASHSCOPE_MODEL: str = "qwen-plus"

    # 数据库配置
    DATABASE_URL: str = "postgresql+psycopg://maillife:maillife@localhost:5432/maillife"
    ...
    # MCP Notion 配置（Stdio 方式）
    MCP_NOTION_COMMAND: str = "npx"
    MCP_NOTION_ARGS: str = "-y,@modelcontextprotocol/server-notion"
    MCP_NOTION_CWD: str = "."

    # Notion 配置（用于邮件摘要归档）
    NOTION_API_KEY: str = ""  # 内部集成密匙 (Internal Integration Token)
    NOTION_DATABASE_ID: str = ""  # 用于归档的数据库 ID
