    ...
    def execute(self, input_data: Any, **kwargs) -> AgentResult:
        ...
        try:
            ...
            # 如果是其他输入，构建查询参数
                ...
                db = SessionLocal()
                try:
                    emails = db.query(Email).filter(Email.is_processed == False).limit(limit).all()
                    parsed_count = 0
                    for email in emails:
...
class SummarizerAgent(BaseAgent):
    ...
    def execute(self, input_data: Any, **kwargs) -> AgentResult:
        ...
        try:
            ...
            try:
                # 获取最近邮件和事件
                emails = db.query(Email).order_by(Email.date.desc()).limit(20).all()
                ...
                # 事件摘要
                if events:
                    important_events = [e for e in events if e.status == "important"]
