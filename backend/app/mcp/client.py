...
class StdioMCPClient(MCPClientBase):
    ...
    def __init__(self, command: str, args: List[str], name: str = "", cwd: str = None, env: Dict[str, str] = None):
...
class MCPClientManager:
    ...
    def __init__(self):
        ...
        self.logger = Logger.get("mcp_manager")
    ...
    def add_stdio_client(self, name: str, command: str, args: List[str], cwd: str = None, env: Dict[str, str] = None) -> Optional[StdioMCPClient]:
        """添加 Stdio MCP 客户端"""
        client = StdioMCPClient(command, args, name, cwd, env)
        if client.connect():
            self.clients[name] = client
    ...
    def add_client(self, name: str, server_url: str = None, command: str = None, args: List[str] = None, cwd: str = None, env: Dict[str, str] = None) -> Optional[MCPClientBase]:
        """通用添加客户端（自动识别类型）"""
        if server_url:
            return self.add_http_client(name, server_url)
        elif command and args:
            return self.add_stdio_client(name, command, args, cwd, env)
        else:
